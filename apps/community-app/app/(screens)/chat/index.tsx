...
const ChatListRow = (props: ChatListRowProp) => {
  ...
  const receiver = props.members.find(
  ...
  return (
    <Pressable
      ...
      className="flex-row gap-4 items-center w-full"
    >
      <Image
...
export default function Chat() {
  ...
  const { data: roomsRes, isLoading: isRoomsResLoading } = useQuery({
    ...
    queryFn: () =>
      fetch(
        `${directusUrl}/items/rooms?[members][_contains]=${userData?.id}&fields=id,members.directus_users_id.first_name,members.directus_users_id.id,members.directus_users_id.avatar`,
        {
          headers: {
