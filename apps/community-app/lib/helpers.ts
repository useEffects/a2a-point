...
export const getDMRoomId = async ([userId1, userId2]: [string, string], access_token: string) => {
    const room = await fetch(`${directusUrl}/items/rooms?[members][_contains]=${userId1}&[members][_contains]=${userId2}&[isGroup][_eq]=false`, {
        headers: {
    ...
    if (!room.data || !room.data.length || !room.data[0].members.length) {
        ...
        const res = await fetch(`${directusUrl}/items/rooms_directus_users`, {
            ...
            body: JSON.stringify([userId1, userId2].map(userId => ({
                rooms_id: roomId,
