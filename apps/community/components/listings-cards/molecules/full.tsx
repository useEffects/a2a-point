...
export type FullListingDetailed = Listing & { user_created: Pick<User, "id" | "avatar" | "first_name" | "email"> } & { amenities: DetailedAmenity[] }
...
export const FullListingCard = (props: FullListingDetailed) => {
    return <View className="flex-col gap-4 p-2">
        ...
        <View className="flex-row gap-4 items-center">
            ...
            <View className="flex-col gap-2">
                <View>
                    <Text className="text-lg font-medium">{props.user_created.first_name}</Text>
                    <Text className="text-muted-foreground">{props.user_created.email}</Text>
                </View>
                <View className="flex-row gap-2">
                    <Button size={"sm"} variant={"outline"}><Text className="!text-sm">Chat</Text></Button>
                    <Button size={"sm"} variant={"outline"}><Text className="!text-sm">Profile</Text></Button>
                </View>
            </View>
        </View>
        <Hr />
        <View className="flex-row gap-4 justify-around">
            <View className="flex-col gap-2 items-center">
                <Feather name="eye" className="!text-foreground" size={18} />
                <Text className="text-sm text-muted-foreground">43 Views</Text>
            </View>
            <View className="flex-col gap-2 items-center">
                <FontAwesome name="comments" size={18} className="!text-foreground" />
                <Text className="text-sm text-muted-foreground">43 Comments</Text>
            </View>
            <View className="flex-col gap-2 items-center">
                <Feather name="bookmark" className="!text-foreground" size={18} />
                <Text className="text-sm text-muted-foreground">43 Saves</Text>
            </View>
        </View>
    </View>
}