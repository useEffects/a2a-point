...
import { getDMRoomId, shortString } from "~/lib/helpers"
...
export const MediumListingCard = (item: MediumListingCardProps) => {
    ...
    return <View className="w-full flex-col gap-2 px-2 my-6">
        <View className="flex flex-row items-center justify-between">
            ...
            {user.id === item.user_created.id ? <></> : <Button size="none" variant="base" onPress={async () => router.navigate(`/chat/${await getDMRoomId([user.id, item.user_created.id])}`)}>
