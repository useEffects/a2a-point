...
function Form1({
  ...
  setFormValues: Dispatch<SetStateAction<Form1Values>>;
  setNavigationState: Dispatch<SetStateAction<NavigationState<Route>>>;
}) {
  ...
  const Form = (props: FormikProps<Form1Values>) => {
    ...
    return (
      <ScrollView contentContainerClassName="flex-grow justify-between">
        <View className="flex-col gap-4">
          ...
          <FormInput
            ...
            error={
              props.touched.expectedBrokerFees
                ? props.errors.expectedBrokerFees
                : ''
            }
          />
          <FormInput
            ...
            onChangeText={(val) =>
              props.setFieldValue('size', parseInt(val.replace(/,/g, '')))
            }
...
function Form2({
  ...
  setFormValues: Dispatch<SetStateAction<Form2Values>>;
  setNavigationState: Dispatch<SetStateAction<NavigationState<Route>>>;
}) {
  ...
  const Form = (props: FormikProps<Form2Values>) => {
        ...
        <View className="flex-col gap-1">
          ...
          {tags.length ? (
            <View className="flex-row flex-wrap gap-1">
              {tags.map((tag, index) => (
                <Button
                  disabled={!Boolean(currentTag) || Boolean(error)}
                  className="self-start bg-secondary"
                  variant={'base'}
                  size={'none'}
                  onPress={() => setTags(tags.filter((t, i) => i !== index))}
                  key={index}
                >
                  <Text className="text-secondary-foreground text-xs rounded p-1">
                    {tag}
                  </Text>
                </Button>
              ))}
    ...
    const AmenitiesInput = ({
      ...
      setDetailedAmenities: (newDetailedAmenities: DetailedAmenity[]) => void;
    }) => {
      const [currentAmenity, setCurrentAmenity] = useState<Amenity | null>(
        null,
      );
      const [currentAdditionalDetail, setCurrentAdditionalDetail] =
        useState('');
        ...
        <View className="flex-col gap-8">
          ...
          <View className="flex-col gap-4">
            ...
            <FormInput
              ...
              rightComponent={() => (
                <Button
                  disabled={!Boolean(currentAmenity)}
                  onPress={() => {
                    setDetailedAmenities([
                      ...detailedAmenities,
                      {
    ...
    return (
      <ScrollView contentContainerClassName="flex-grow">
        <View className={cn('flex-1 flex-col gap-4 justify-start')}>
          <View className="flex-col gap-4">
            <Text className="my-4">
              Optional fields that would boost user interactions
            </Text>
            <TagInput
            ...
            <AmenitiesInput
              detailedAmenities={props.values.amenities}
              setDetailedAmenities={(newDetailedAmenities: DetailedAmenity[]) =>
                props.setFieldValue('amenities', newDetailedAmenities)
              }
            />
          </View>
...
function Form3({
  ...
  setFormValues: Dispatch<SetStateAction<Form3Values>>;
  setNavigationState: Dispatch<SetStateAction<NavigationState<Route>>>;
}) {
  ...
  return (
    <ScrollView contentContainerClassName="flex-col gap-8 flex-grow">
      ...
      ].map(({ label, photo, key }, index) => (
        <View className="flex-col gap-4" key={index}>
          ...
          <View className="flex-row gap-4">
            ...
            {photo && (
              <Button
                onPress={() => setFormValues((p) => ({ ...p, [key]: null }))}
                variant={'destructive'}
                size={'sm'}
                className="w-1/2"
              >
...
function Form4({
  ...
  setFormValues: Dispatch<SetStateAction<Form4Values>>;
  handleSubmit: () => void;
  loading: boolean;
}) {
    ...
    return (
      <ScrollView contentContainerClassName="flex-grow">
        <View className="flex-1 flex-col justify-center items-center gap-12">
          ...
          <Text
            className={cn(
              user.premium_quota ? 'text-success' : 'text-destructive',
            )}
          >
            you have {user.premium_quota} premium listing cap available
          </Text>
          {user.premium_quota ? (
            <View className="flex-row gap-4 items-center">
              ...
              <Switch
                ...
                onValueChange={(val) => setFormValues({ featured: val })}
              />
            </View>
          ) : (
            <></>
        ...
        <View className="mt-1/2 mb-4 flex-col items-center gap-4 flex-1 justify-center">
          ...
          <Text className="text-center text-warning">
            The listings you create will be added to a verification queue, where
            our staff will review and approve them.
          </Text>
        </View>
...
function PostScreenComponent() {
  const [form1Values, setForm1Values] =
    useState<Form1Values>(form1InitialValues);
  const [form2Values, setForm2Values] =
    useState<Form2Values>(form2InitialValues);
  const [form3Values, setForm3Values] =
    useState<Form3Values>(form3InitialValues);
  const [form4Values, setForm4Values] =
    useState<Form4Values>(form4InitialValues);
  ...
  const handleSubmit = async () => {
    ...
    const assets = [
      ...
      form3Values.photo_2,
      form3Values.photo_3,
    ].filter((asset) => asset !== null) as Asset<withUri>[];
    const assetsId = await Promise.all(
      assets.map((asset) => uploadFileToDirectus(asset, listingsFolderId)),
    );

    const payload: Partial<
      Omit<Listing, 'amenities'> & {
        amenities: {
...
export const RenderAmenity = ({
  ...
  return (
    <View className="w-full border border-border rounded-2xl flex-col gap-4 p-4 bg-card">
      ...
      {additional_detail ? (
        <Text className="text-subtext text-sm">{additional_detail}</Text>
      ) : (
        <></>
      )}
    </View>
...
const RenderDealTypeSpecificComponent = ({
  ...
  Form1Values,
  'deal_type' | 'covered_by_seller' | 'furnishing' | 'budget' | 'price'
> & {
  ...
  switch (deal_type) {
    case 'sale':
      return (
        <>
          <FormSelect
            options={['yes', 'no', 'dependent'].map((val) => ({
              label: val,
              value: val,
            }))}
            ...
            value={
              covered_by_seller
                ? { value: covered_by_seller, label: covered_by_seller }
                : undefined
            }
            onValueChange={(val) =>
              setFieldValue('covered_by_seller', val?.value)
            }
          />
          <FormInput
            label="Price (AED)"
            ...
            onChangeText={(val) =>
              setFieldValue('price', parseInt(val.replace(/,/g, '')))
            }
    ...
    case 'give on rent':
      return (
        <>
          <FormSelect
            options={['furnished', 'semi furnished', 'unfurnished'].map(
              (val) => ({ label: val, value: val }),
            )}
            label="Furnishing"
            value={
              furnishing ? { value: furnishing, label: furnishing } : undefined
            }
    ...
    case 'take on rent':
      return (
        <FormInput
          ...
          onChangeText={(val) =>
            setFieldValue('budget', parseInt(val.replace(/,/g, '')))
          }
...
export const RenderAmenities = ({
  ...
  return (
    <View className="">
      {groupByN(amenities).map((_amenities, i) => (
        <View key={i} className="flex-row gap-4 w-full">
          {_amenities.map((amenity, j) => (
