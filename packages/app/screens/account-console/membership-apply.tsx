...
export const MembershipApplyScreen = () => {
  ...
  const checkoutWithCoupon = async () => {
    ...
    if (isValid) {
      setError('');
      Linking.openURL(
        `${portfolioUrl}/api/pay/${basicPlanStripeCodes.monthly}/?mode=subscription&coupon=${code}&user_id=${user.id}&isMobile=true`,
      );
    } else {
            ...
            <Card className="mb-0 mt-auto">
              ...
              <CardFooter>
                <Text className="text-subtext">
                  For invoice or billing details contact support
                </Text>
              </CardFooter>
            </Card>
          </View>
          ...
          <View className="flex-col gap-8">
            ...
            <View className="flex-col gap-4">
              <View>
                <Text className="text-success">
                  Have access code from your company?
                </Text>
            ...
            <View className="flex-col gap-2 items-end">
              ...
              <Text className="text-info text-sm">
                Buy yearly plans at discounted prices
              </Text>
            </View>
            {membershipCardItems.map((membershipCardItem, i) => (
              <MembershipCard
                key={i}
                {...membershipCardItem}
                isYearly={yearly}
...
export const MembershipCard = ({
  ...
  isStatic,
  isYearly,
}: MembershipCardProps & { isStatic?: boolean; isYearly?: boolean }) => {
  const { user } = userStore();
  ...
  const checkout = async () => {
    ...
    console.log(link);
    Linking.openURL(
      `${portfolioUrl}/api/pay/${priceId}/?mode=subscription&user_id=${user.id}&isMobile=true`,
    );
  };
  ...
  return (
    <View
      className={cn('flex-row gap-8 p-4 rounded-xl border border-border', bg)}
    >
      <View className="w-36 flex-col gap-4">
        <Text className={cn('font-medium text-lg', fg)}>{name}</Text>
        {!discount ? (
          <Text>AED {amount}</Text>
        ) : (
          <View>
            <View className="flex-row gap-1 items-center">
              <Text className={cn(fg)}>AED</Text>
              <Text className={cn('line-through', fg)}>{amount}</Text>
              <Text className={cn('font-medium text-xl', fg)}>
                {discountedAmount}
              </Text>
            </View>
            {discountReason && (
              <Text className={cn(fg)}> {discountReason} </Text>
            )}
          </View>
        )}
      </View>
      <View className="flex-col gap-4 flex-grow">
        ...
        {!isStatic ? (
          <Button onPress={checkout} variant={'secondary'} size={'sm'}>
