...
import {
  ...
  mediumCardListingsWithAds,
  RenderMediumListingsAds,
} from 'app/components/cards/molecules2/listings';
import {
  FormAutoSelect,
  RenderCompanyTileProps,
  RenderListingTileProps,
  RenderUserTileProps,
  useAutoCompleteItem,
...
export function ListingsScreen({
  ...
  const updateParams = function (filters: Filter[]) {
    ...
    const newFilters = filters.reduce<{ [key in FilterKeys]?: FilterValue }[]>(
    ...
    <View className={cn('flex-1', className)}>
      ...
      >
        <View className="py-4 flex-col gap-8 bg-card w-full">
          ...
          <View className="px-4">
            {isDifferent(currentFilters, filters) ? (
              <Button onPress={applyFilters} variant={'secondary'}>
                <Text>Apply</Text>
              </Button>
            ) : (
              <></>
  ...
  return (
    <View className="w-full flex-row justify-between py-4">
      {props.navigationState.routes.map((route, i) => {
        ...
        return (
          <Button
            className="flex-grow flex-col gap-2 relative"
            key={i}
            variant={'base'}
            size="none"
            onPress={() =>
              props.setNavigationState((p) => ({ ...p, index: i }))
            }
          >
            <Svg
              height={8}
...
const RangeSliderScenes = (
  ...
  const cost =
    (props.filters.find((f) => f.key === FilterKeys.Cost)?.value as [
      number,
      number,
    ]) || costRange;
  const size =
    (props.filters.find((f) => f.key === FilterKeys.Size)?.value as [
      number,
      number,
    ]) || sizeRange;
  const bedRooms =
    (props.filters.find((f) => f.key === FilterKeys.Bedrooms)?.value as [
      number,
      number,
    ]) || bedRoomsRange;
  const bathRooms =
    (props.filters.find((f) => f.key === FilterKeys.Bathrooms)?.value as [
      number,
      number,
    ]) || bathRoomsRange;
  const parking =
    (props.filters.find((f) => f.key === FilterKeys.Parking)?.value as [
      number,
      number,
    ]) || parkingRange;

  const dispatcher =
    (key: FilterKeys, range: [number, number]) => (value: [number, number]) => {
      if (props.filters.some((f) => f.key === key)) {
        if (value[0] === range[0] && value[1] === range[1]) {
          props.setFilters(props.filters.filter((f) => f.key !== key));
        } else {
          props.setFilters(
            props.filters.map((f) => (f.key === key ? { key, value } : f)),
          );
        }
      } else {
    ...
    case FilterKeys.Cost:
      return (
        <RangeFilter
          ...
          range={costRange}
          value={cost}
          setValue={dispatcher(FilterKeys.Cost, costRange)}
        />
      );
    case FilterKeys.Size:
      return (
        <RangeFilter
          ...
          range={sizeRange}
          value={size}
          setValue={dispatcher(FilterKeys.Size, sizeRange)}
        />
      );
    case FilterKeys.Bedrooms:
      return (
        <RangeFilter
          ...
          range={bedRoomsRange}
          value={bedRooms}
          setValue={dispatcher(FilterKeys.Bedrooms, bedRoomsRange)}
        />
      );
    case FilterKeys.Bathrooms:
      return (
        <RangeFilter
          ...
          value={bathRooms}
          setValue={dispatcher(FilterKeys.Bathrooms, bathRoomsRange)}
        />
      );
    case FilterKeys.Parking:
      return (
        <RangeFilter
          ...
          range={parkingRange}
          value={parking}
          setValue={dispatcher(FilterKeys.Parking, parkingRange)}
        />
      );
  }
};
...
const RangeFilter = ({
  ...
  return (
    <View className="flex-col gap-2 w-full h-full justify-center">
      <View className="flex-row gap-2 items-center justify-between">
        <Text>{label}</Text>
        <Text className="text-info">
          {localizedMin} - {localizedMax}
          {isAtMax ? '+' : ''}
        </Text>
      </View>
      <RangeSlider
...
const ComboBoxFilters = ({
  ...
  const company = useAutoCompleteItem(
    'companies',
      ...
      <FormAutoSelect
        ...
        setCurrentItem={(item) =>
          item
            ? setFilters([
                ...filters,
                { key: FilterKeys.Location, value: item.id as string },
              ])
            : setFilters(filters.filter((f) => f.key !== FilterKeys.Location))
        }
        filter={{
          type: {
            _eq: 'group',
      ...
      <FormAutoSelect
        ...
        setCurrentItem={(item) =>
          item
            ? setFilters([
                ...filters,
                { key: FilterKeys.Agent, value: item.id as string },
              ])
            : setFilters(filters.filter((f) => f.key !== FilterKeys.Agent))
        }
      />
      <FormAutoSelect
        item="companies"
        ...
        setCurrentItem={(item) =>
          item
            ? setFilters([
                ...filters,
                { key: FilterKeys.Company, value: item.id as string },
              ])
            : setFilters(filters.filter((f) => f.key !== FilterKeys.Company))
        }
      />
    </View>
  );
    ...
    <View className="flex-row justify-between w-full">
      {categoryTiles.map((category, i) => (
        ...
        >
          {category.Icon({
            size: 24,
            color: filters.some((f) => f.key === category.key)
              ? colors.primary
              : colors.foreground,
          })}
          <Text
...
const RenderChips = ({
  ...
  const shouldRenderPlus = (max: number, value: number) =>
    value === max ? '+' : '';
  ...
  const location = useAutoCompleteItem(
    'rooms',
    locationId?.toString(),
  ) as RenderListingTileProps | null;
  const agent = useAutoCompleteItem(
    'users',
    agentId?.toString(),
  ...
  const company = useAutoCompleteItem(
    'companies',
    companyId?.toString(),
  ) as RenderCompanyTileProps | null;

  const getRangeSliderLabel = (
    key: FilterKeys,
    value: [number, number],
    maxRange: number,
  ) => {
    const [min, max] = value;
    return key === FilterKeys.Cost
      ? `${key}: AED ${min.toLocaleString()} - AED ${max.toLocaleString()}${shouldRenderPlus(maxRange, max)}`
      : `${key}: ${min.toLocaleString()} - ${max.toLocaleString()}${shouldRenderPlus(maxRange, max)}`;
  };
