...
import { ProductType, appName, DIRECTUS_URL, products } from './constants';
...
export const checkCollectionId = async (
  ...
  try {
    const res = await fetch(
      `${DIRECTUS_URL}/items/${collection}/${id}/?fields=${id}`,
      {
        headers: {
...
export const getCostOfListing = (
  dealType: string,
  budget: number,
  price: number,
) => {
  const _budget = budget.toLocaleString('en-US');
  const _price = price.toLocaleString('en-US');
  return dealType === 'buy' || dealType === 'take on rent' ? _budget : _price;
};
