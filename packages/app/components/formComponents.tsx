...
export const FormInput = (props: TextInputProps & AdditionalFormInputProps) => {
  ...
  const handleSizeChange = (newHeight: number) => {
    if (
      newHeight <= initialInputHeight * maxLines &&
      ...
      <View className="flex-row justify-between items-center">
        {props.label && (
          <Text
            className={cn(
              'text-sm',
              error?.length ? 'text-destructive' : 'text-subtext',
            )}
          >
            {label}
          </Text>
        )}
        {props.autoSelect && error && (
      ...
      <View className="flex-row gap-4 items-center">
        <Input
          ...
          style={{
            ...
            borderColor: error?.length ? colors.destructive : colors.border,
          }}
          onContentSizeChange={(e) =>
            handleSizeChange(e.nativeEvent.contentSize.height)
          }
          multiline={Platform.OS !== 'web'}
          className={cn(
            'text-base flex-grow',
            props.readOnly && 'text-subtext',
            rest.className,
          )}
        />
        <RightComponent />
...
export const FormSelect = (
  props: SelectRootProps & AdditionalFormSelectProps,
) => {
  ...
  return (
    <View className="flex-col gap-2">
      ...
      <Select>
        <SelectTrigger
          className={cn(error ? 'border-destructive' : 'border-border')}
        >
          <SelectValue
        ...
        <SelectContent className="w-60" sideOffset={-40 + 8}>
          <SelectGroup>
            {props.options.map((option, index) => (
              <SelectItem key={index} value={option.value} label={option.label}>
...
export type AutoCompleteRenderItemProps =
...
export const useAutoCompleteItem = (
  ...
  const { data } = useQuery<AutoCompleteRenderItemProps | null>({
    ...
    queryFn: async () => {
      ...
      return item === 'users'
        ? await fetch(
            `${directusUrl}/users/${id}?fields=${autoCompleteFields[item].join(',')}`,
            {
              headers: {
            ...
            .then((res) => res.json())
            .then((res) => res.data as RenderUserTileProps)
        : ((await rest.request(
            readItem(item, id!, {
              filter: filter,
...
export const FormAutoSelect = (
  ...
  const isRenderListingTile = (
    item: AutoCompleteRenderItemProps | undefined,
  ): item is RenderListingTileProps => {
    return props.item === 'listings';
  };
  const isRenderCompanyTile = (
    item: AutoCompleteRenderItemProps | undefined,
  ): item is RenderCompanyTileProps => {
  ...
  const RenderItem = (item: AutoCompleteRenderItemProps) => {
    if (isRenderUserTile(item)) {
      return (
        <RenderUserTile
          {...item}
          currentId={props.currentItem?.id as string | undefined}
        />
      );
    }
    ...
    if (isAmenity(item)) {
      return (
        <RenderAmenityTile
          {...item}
          currentId={props.currentItem?.id as number | undefined}
        />
      );
    }
  };
  ...
  return (
    <View className="w-full">
      <FormInput
        ...
        className={cn(
          showResults && 'rounded-b-none outline-none',
          props.className,
        )}
...
const RenderRoomTile = (
  props: RenderRoomTileProps & { currentId: string | undefined },
) => {
  return (
    <View
      className={cn(
        'flex-row p-2 rounded gap-4 items-center',
        props.currentId === props.id && 'bg-popover flex-1 w-full',
      )}
    >
      <AsyncImage
...
const RenderListingTile = (
  props: RenderListingTileProps & { currentId: string | undefined },
) => {
  return (
    <View className="flex-col p-2 gap-2 items-start">
...
const RenderUserTile = (
  props: RenderUserTileProps & { currentId: string | undefined },
) => {
  return (
    <View
      className={cn(
...
const RenderCompanyTile = (
  props: RenderCompanyTileProps & { currentId: string | undefined },
) => {
  return (
    <View
      className={cn(
...
const RenderAmenityTile = (
  props: Amenity & { currentId: number | undefined },
) => {
  const { colors } = useColorScheme();
