...
export type Asset<T extends withId | withUri> = T & {
  mimeType: string;
  name: string;
};

export type ChatMessage<T extends withId | withUri> = Omit<
  Message,
  'user_created' | 'assets'
> & {
  user_created: Pick<
    User,
    'first_name' | 'id' | 'last_name' | 'avatar' | 'plan'
  >;
} & { sent: boolean; assets?: Asset<T>[] };
...
type ChatUiProps = {
  ...
  isGroup?: boolean;
  listProps?: Omit<
    SectionListProps<ChatMessage<withId | withUri>>,
    'sections' | 'renderItem'
  >;
  receivers: Member[];
};
...
export const ChatBubble = (
  props: ChatMessage<withId | withUri> & { currentUserId: string } & {
    goToId?: string;
    isFirst: boolean;
    isLast: boolean;
    isGroup?: boolean;
  ...
  const additionalSpacing = renderRight
    ? isTextBig
      ? 'mr-0 mb-1'
      : 'mr-2'
    : isTextBig
  ...
  const containerStyle = renderRight
    ? 'bg-primary flex-start'
    : 'bg-secondary flex-end';
  const roundedStyle = renderRight
    ? cn(
        'rounded-tl-2xl rounded-bl-2xl',
        props.isFirst ? 'rounded-br-2xl' : '',
        props.isLast ? 'rounded-tr-2xl' : '',
      )
    : cn(
        'rounded-tr-2xl rounded-br-2xl',
        props.isLast ? 'rounded-tl-2xl' : '',
  ...
  return (
    <View
      className={cn(
        'px-4 native:px-0',
        toHighlight && 'bg-accent',
        'mt-[1px]',
        props.isGroup && 'flex-col gap-1',
        props.isFirst && 'mb-2',
        props.isLast && 'mt-2',
      )}
    >
      ...
      <View
        className={cn(
          'py-2 px-4 items-center max-w-[90%] md:max-w-[50%]',
          containerStyle,
          roundedStyle,
          flexDirection,
          marginDirection,
      ...
      >
        <View className="flex-col">
          {props.assets && props.assets.length ? (
            <ImageGroup assets={props.assets} />
          ) : (
            <></>
          )}
          {props.content ? (
        ...
        <View className={cn('flex-row gap-1 items-center', infoPositioning)}>
          <Text className={cn('text-xs font-light', textColor)}>
            {shortTime(props.date_created)}
          </Text>
          {renderRight ? (
            props.sent ? (
...
const FooterDropDownMenu = (props: {
  open: boolean;
      ...
      const _assets = result.assets
        .map((asset) => {
          return {
            ...
            name:
              type === 'document'
                ? (asset as DocumentPicker.DocumentPickerAsset).name
        ...
        .filter(async (asset) => {
          ...
          if (fileSize > 1 * 1024 * 1024) {
            alert(
              `File size exceeds 1MB limit for ${asset.name} (${filesize(fileSize)})`,
            );
            return false;
          }
          return true;
  ...
  return (
    <DropdownMenu onOpenChange={props.setOpen}>
...
const Footer = (
  props: Pick<
    ChatUiProps,
    'currentMessage' | 'currentMessageDispatcher' | 'onSend' | 'isGroup'
  > & { receivers: Member[] },
) => {
  const { colors } = useColorScheme();
        ...
        <ScrollView contentContainerClassName="flex-grow flex-col gap-4">
          <View className="flex-col gap-4">
            ...
            <FormAutoSelect
              ...
              setCurrentItem={(item) =>
                formProps.setFieldValue('listing', item)
              }
              label="Listing"
              ...
              filter={{
                _or: [
                  ...
                  {
                    user_created: {
                      _eq: props.receivers.find(
                        (r) => r.directus_users_id.id !== user.id,
            ...
            <FormInput
              ...
              error={
                formProps.touched.commissionSeller
                  ? formProps.errors.commissionSeller
                  : ''
              }
            ...
            <FormInput
              ...
              error={
                formProps.touched.commissionBuyer
                  ? formProps.errors.commissionBuyer
                  : ''
              }
              keyboardType="numeric"
            ...
            <FormInput
              ...
              label="Client Name"
              error={
                formProps.touched.clientName ? formProps.errors.clientName : ''
              }
    ...
    >
      ...
      {currentMessage.assets && currentMessage.assets.length ? (
        <View className="border-solid border-0 border-l-4 border-primary bg-accent p-2 flex-row justify-between items-center">
          ...
          <Button
            onPress={() => currentMessageDispatcher((p) => ({ text: p.text }))}
            size={'icon'}
      ...
      <View className="flex-row gap-4 native:h-16 h-14 w-full py-2 px-4 items-center bg-accent">
        {disabled && (
          <>
            <FooterDropDownMenu
              open={open}
              setOpen={setOpen}
              currentMessageDispatcher={currentMessageDispatcher}
            />
            {!props.isGroup ? (
              <Button
        ...
        <Input
          ...
          onChangeText={(newVal) =>
            currentMessageDispatcher((p) => ({ ...p, text: newVal }))
          }
...
function formatDate(date: Date) {
  ...
  if (diffInDays === 0) {
    ...
    // -6 to 0 means within the last 6 days
...
const generateSections = (messages: ChatMessage<withId | withUri>[]) => {
  const sectionMap = messages.reduce<{
    [key: string]: {
      timeStamp: string;
      messages: ChatMessage<withId | withUri>[];
    };
  ...
  return Object.keys(sectionMap).map((date) => ({
    title: formatDate(new Date(sectionMap[date]!.timeStamp)),
    data: sectionMap[date]!.messages,
  }));
};
...
export const ChatUi = (props: ChatUiProps) => {
  ...
  const sections = useMemo(
    () => generateSections(uniqBy(props.messages, 'id')),
    [props.messages],
  );
  const { colors } = useColorScheme();
    ...
    if (props.goToId) {
      const { foundItemIndex, foundSectionIndex } = sections.reduce(
        (acc, section, sectionIndex) => {
          const foundItemIndex = section.data.findIndex(
            (item) => item.id === props.goToId,
          );
          if (foundItemIndex !== -1) {
      ...
      <SectionList
        ...
        renderItem={({ item, index, section }) => (
          <ChatBubble
            ...
            isFirst={
              index === 0 ||
              section.data[index - 1]?.user_created.id !== item.user_created.id
            }
            isLast={
        ...
        renderSectionFooter={({ section }) => (
          <Text className="text-sm text-center text-subtext py-4">
            {section.title}
          </Text>
        )}
