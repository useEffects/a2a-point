...
export const mediumCardListingsWithAds = async (limit: number, page: number): Promise<((MediumListingCardProps & ListingCardMetrics) | ConfirmedAdvertisementCardProps)[]> => {
    ...
    const ads = await renderCardsQuery<AdvertisementCardProps>({
        ...
        sort: ["-date_created"],
        limit: adsLimit,
        offset: page * adsLimit,
    }).then(res => res.map(r => ({ ...r, [IS_AD_TYPE]: true })))

    return mergeArraysRandomly(listings, ads)
}

export const RenderMediumListingsAds = ({ item }: { item: ((MediumListingCardProps & ListingCardMetrics) | ConfirmedAdvertisementCardProps) }) => {
