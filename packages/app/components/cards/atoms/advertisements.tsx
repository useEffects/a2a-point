...
export type AdvertisementCardProps = Pick<Advertisement, "id" | "caption" | "title" | "photo" | "date_created" | "link_to_open"> & { user_created: Pick<User, "id" | "first_name" | "last_name" | "email" | "avatar" | "plan"> }
...
export const AdvertisementCard = (props: AdvertisementCardProps) => {
    ...
    >
        ...
        <Image source={{ uri: buildAssetUrl(props.photo) }} className="w-full aspect-video" />
        <Text className="text-sm px-4">{props.caption}</Text>
        <Text className="text-xs text-subtext px-4">{timeAgo.format(new Date(props.date_created))}</Text>
    </Pressable>
}