...
import { getCostOfListing } from 'app/lib/helpers';
import { Listing } from 'app/lib/types';
import { useRouter } from 'expo-router';
import { Pressable, View } from 'react-native';

export type ExtraSmallListingCardProps = Pick<
  Listing,
  'id' | 'title' | 'budget' | 'price' | 'deal_type'
>;

export const ExtraSmallListingCard = (props: ExtraSmallListingCardProps) => {
  const router = useRouter();
  const localizedCost = getCostOfListing(
    props.deal_type,
    props.budget,
    props.price,
  );
