...
const LocationCard = ({ item }: { item: Pick<Room, "id" | "title" | "avatar"> }) => {
    ...
    return <View className="flex-col gap-4 items-center">
        <Button onPress={() => router.push({ pathname: "/discover", query: { id: item.id, filter: CommonFilters.GroupId } })} size={"icon"} className="relative h-16 w-16 rounded-full" variant={"ghost"}>
