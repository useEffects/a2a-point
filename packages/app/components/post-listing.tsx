...
function Form1({
  ...
  const dealTypeOptions = Object.entries(dealTypeLabels).map(
      ...
      <ScrollView contentContainerClassName="flex-grow">
        <View className="flex-1 flex-col gap-4 px-4">
          ...
          <FormSelect
            label="Deal Type"
            value={{
              label: dealTypeLabels[props.values.dealType],
              value: props.values.dealType,
            }}
            onValueChange={(e) =>
              e?.value && props.setFieldValue('dealType', e.value)
            }
          ...
          <FormSelect
            label="Type"
            value={{
              label: typeLabels[props.values.type],
              value: props.values.type,
            }}
            onValueChange={(e) =>
              e?.value && props.setFieldValue('type', e.value)
            }
            options={typeOptions}
...
function Form2({
  ...
  const Form2Schema = Yup.object().shape({
    tags: Yup.array().of(Yup.string().min(3).max(20)),
...
function Form3({
  ...
  initialValues?: { featured: boolean };
}) {
...
export default function PostListing() {
  ...
  >({
    index: 0,
    routes: [{ key: 'form1' }, { key: 'form2' }, { key: 'form3' }],
  });

  const onSubmit = () => {};

  return (
    <View className="flex-1 w-full flex-col gap-4">
      <TabView
        swipeEnabled={false}
        renderTabBar={() => null}
