...
const ImageTile = ({
  ...
  asset: Asset<withId | withUri>;
  full?: boolean;
}) => {
  const handleOnPress = async (asset: Asset<withId | withUri>) => {
    if ('id' in asset) {
      buildAssetUrl(asset.id).then(Linking.openURL);
    }
  };
  return imageMimeTypes.includes(asset.mimeType) ? (
    full ? (
      <Button
...
export const ImageGroup = ({
  ...
  return (
    <Button
      ...
      className="flex-row gap-1 mb-2 items-end"
    >
      {_assets.map((asset, key) => (
        <ImageTile key={key} asset={asset} />
      ))}
      {assets.length > _assets.length ? (
        <Text className="bg-accent text-accent-foreground px-1 rounded-full text-xs">
      ...
      >
        <View className="p-2 bg-popover flex-col">
          <FlatList
            ...
            ListHeaderComponent={
              <View className="my-4 flex-row gap-2 items-center">
                <Button
                  variant={'ghost'}
