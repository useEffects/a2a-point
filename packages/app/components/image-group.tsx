...
const ImageTile = ({ asset, full = false }: { asset: Asset<withId | withUri>, full?: boolean }) => {
    ...
    return imageMimeTypes.includes(asset.mimeType) ? full ? <FullWidthImage source={{ uri: getUrl(asset) }} /> : <Image className="w-28 h-28 object-contain rounded" source={{ uri: getUrl(asset) }} />
