...
export type MessageDetailed = Pick<
  Message,
  'id' | 'room' | 'content' | 'date_created'
> & {
  user_created: Pick<
    User,
    'id' | 'first_name' | 'avatar' | 'last_name' | 'plan'
  >;
} & {
  assets: {
...
export const ChatsProvider = ({ children }: { children: ReactNode }) => {
        ...
        data.data.forEach((message) => {
          if (message.user_created.id === user.id) {
            ...
            if (!sentNotifications[message.room]) {
              const members = roomsSubscribed?.find(
                (room) => room.id === message.room,
              )?.members;
              const recipients =
                members
                  ?.filter((member) => member.directus_users_id.id !== user.id)
                  .map((member) => member.directus_users_id.id) ?? [];
  ...
  useEffect(() => {
    roomsSubscribed.forEach(async (roomSubscribed) => {
      ...
      if (data) {
        setMessages((messages) => [
          ...data.map((message) => transformMessage(message, true)),
          ...messages,
        ]);
      }
    });
    initializeWebSocket().then(setWs);
    return () => {
  ...
  useEffect(() => {
    async function sendMessages() {
      const unsentMessages = messages.filter(
        (m) => !m.sent,
      ) as ChatMessage<withUri>[];
      unsentMessages.forEach(async (message) => {
        const fileIds = await Promise.all(
          message.assets?.map((asset) =>
            uploadFileToDirectus(asset, messagesFolderId),
          ) ?? [],
        );
        const payload = {
          id: message.id,
  ...
  return (
    <ChatsContext.Provider
      value={{
        setMessage: (message: ChatMessage<withUri>) =>
          setMessages((messages) => [message, ...messages]),
        addRoom: (roomId: string) =>
          _setRoomsSubscribed(setRoomsSubscribed, roomsSubscribed, roomId),
...
async function _setRoomsSubscribed(
  setRoomsSubscribed: Dispatch<SetStateAction<RoomSubscribed[]>>,
  prevRooms: RoomSubscribed[],
  roomId: string,
): Promise<RoomSubscribed | null> {
  ...
  if (!found) {
    ...
    try {
      const createRoomRes = await queryClient.fetchQuery({
        ...
        queryFn: async () =>
          await rest.request(
            createItem('rooms_directus_users', {
              rooms_id: roomId,
      ...
      queryClient.setQueryData(
        ...
        (rooms: RoomSubscribed[]) => [room, ...rooms],
      );
      return room;
    } catch (error) {
      return null;
