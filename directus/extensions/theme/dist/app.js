...
var opacity$1 = /*@__PURE__*/getDefaultExportFromCjs(lib);

const subdued = (color) => {
  return new Color$1(color).fade(0.5).hex();
};
const accent = (color) => {
  return new Color$1(color).saturate(0.5).hex();
};
const generateTheme = (mode) => {
  const colors = theme$2[mode];
  return defineTheme({
    ...
    rules: {
      ...
      warningBackground: opacity$1(colors.warning, 0.2),
...
const theme$1 = generateTheme("dark");

const theme = generateTheme("light");

const interfaces = [];const displays = [];const layouts = [];const modules = [];const panels = [];const themes = [theme$1,theme];const operations = [];
